...
export class PropertyListComponent implements OnInit {
  ...
  onFiltersChanged(filters: any) {
    this.filteredProperties = this.allProperties.filter(prop => {
      ...
      const matchesCity = filters.city ? prop.location.city.toLowerCase().includes(filters.city) : true;
      ...
      const matchesSearch = filters.search ? prop.generalinfo.title.toLowerCase().includes(filters.search) : true;

      return matchesType && matchesCity && matchesMinPrice && matchesMaxPrice && matchesSearch;
    });
  }
}